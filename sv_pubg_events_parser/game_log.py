...
class GameLogWatcher:
    ...
    def detect_new_logs(self, similarity_threshold: float, frame: np.ndarray):
        ...
        if self.previous_frame is not None:
            ...
            _, thresh1 = cv2.threshold(crop_prev_img, 0, 128,cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
            _, thresh2 = cv2.threshold(crop_cur_img, 0, 128,cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
